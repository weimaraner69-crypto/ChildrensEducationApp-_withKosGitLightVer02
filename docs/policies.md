...
### P-004 Ground Truth 最優先

- 問題生成・解説生成は MEXT ソースのみを参照する。
- 一般知識による補完や推測を正解根拠として扱わない。
- 根拠を提示できない出力は配信しない。
...
- `MEXT Sync Verified` 表示時は、リンク・OCR・ハッシュの3点を必須とする。
- 3点のいずれかが欠落した場合、検証済み扱いにしない。
- 監査ログにソース ID と検証結果を記録する。
...
- 子供アカウント情報は最小限のみ保持する。
- 生年月日は学年判定目的に限定して利用する。
- 学習ログの閲覧権限は本人と保護者のみに限定する。
